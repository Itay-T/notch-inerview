...
Before starting the backend, create `backend/.env` with your OpenAI API key:

```bash
OPENAI_API_KEY=your_openai_api_key
```

`PORT` defaults to `3000`, and `OPENAI_MODEL` defaults to `gpt-4o-mini`, so they are optional.
...
The API tests call OpenAI for the chat response flows, so `backend/.env` needs a valid `OPENAI_API_KEY`.
